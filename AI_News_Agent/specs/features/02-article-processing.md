...
`agent.py` calls `fetch_articles()` once per topic, passing the topic string so it can be set on each `Article`. Cross-topic deduplication happens in `agent.py` before calling this function (URLs already seen under a prior topic are excluded).
...
- If `text` is empty but `snippet` is present, use the snippet prompt.
